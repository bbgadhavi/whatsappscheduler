...
const App: React.FC = () => {
  ...
  const handleRemoveRecipient = useCallback((indexToRemove: number) => {
  ...
  const TabButton: React.FC<{isActive: boolean; onClick: () => void; children: React.ReactNode}> = ({ isActive, children, onClick }) => (
    ...
    >
        {children}
        {isActive && <div className="absolute bottom-0 left-0 right-0 h-1 bg-primary animate-in fade-in duration-300"></div>}
    </button>
  );
  ...
  const renderActiveTab = () => {
     switch (activeTab) {
      case 'scheduler':
        return (
          <div key="scheduler" className="p-4 space-y-6 animate-in fade-in-50 duration-500">
              <Card>
                <div className="p-5 space-y-4">
                  <h3 className="font-bold text-lg text-foreground">Message Template</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <Button onClick={handleSaveTemplate} variant="outline"><ArchiveBoxIcon className="w-5 h-5 mr-2"/>Save</Button>
                    <Button onClick={handleDeleteTemplate} variant="destructiveOutline" disabled={!selectedTemplateName}><TrashIcon className="w-5 h-5 mr-2"/>Delete</Button>
                  </div>
                  <Input
                      id="messageTemplate"
              ...
              <Card>
                <div className="p-5 space-y-4">
                  <h3 className="font-bold text-lg text-foreground">Recipients ({recipients.length})</h3>
                   <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <Button onClick={handleSaveContactGroup} variant="outline"><ArchiveBoxIcon className="w-5 h-5 mr-2"/>Save</Button>
                    <Button onClick={handleDeleteContactGroup} variant="destructiveOutline" disabled={!selectedContactGroupName}><TrashIcon className="w-5 h-5 mr-2"/>Delete</Button>
                  </div>

                  <div className="space-y-2 max-h-48 overflow-y-auto p-2 rounded-lg border bg-muted/50">
                    {recipients.length === 0 ? <p className="text-center text-muted-foreground py-4">No recipients added.</p> 
                    : recipients.map((r, i) => (
                        <div key={r.number} className="flex items-center justify-between p-2 rounded-md bg-card hover:bg-muted/50 animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
                          <div>
                            <p className="font-semibold text-foreground">{r.name}</p>
                            <p className="text-sm text-muted-foreground">+{r.number}</p>
                          </div>
                          <button onClick={() => handleRemoveRecipient(i)} className="text-muted-foreground hover:text-destructive transition-colors p-1 rounded-full"><TrashIcon className="w-5 h-5" /></button>
                        </div>
                    ))}
                  </div>

                  <div className="space-y-4 pt-2">
                    <p className="text-sm font-medium text-muted-foreground uppercase tracking-wide">Add New Contact</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     ...
                     <div className="flex flex-col sm:flex-row gap-4">
                      <Button onClick={handleAddRecipient} variant="outline" className="w-full">Add Manually</Button>
                      <Button onClick={handleImportContacts} variant="outline" className="w-full"><UserPlusIcon className="w-5 h-5 mr-2"/>Import From Contacts</Button>
                    </div>
                  </div>
                </div>
              </Card>
              ...
              <Card>
                <div className="p-5 space-y-3">
                  <h3 className="font-bold text-lg text-foreground">Schedule (Optional)</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      case 'history':
        return (
          <div key="history" className="p-4 space-y-6 animate-in fade-in-50 duration-500">
            <Card className="p-5">
              <div className="flex justify-between items-center mb-4">
                  <h3 className="font-bold text-lg text-foreground">Sent History</h3>
                  <Button onClick={handleClearHistory} variant="destructive" disabled={history.length === 0} className="text-sm py-2">
                      <TrashIcon className="w-5 h-5 mr-2"/> Clear
                  </Button>
              </div>
              <div className="space-y-3 max-h-[calc(100vh-300px)] overflow-y-auto pr-2">
                  {history.length === 0 ? (
                      <p className="text-center text-muted-foreground py-10">No messages in history.</p>
                  ) : (
                      history.map((msg, index) => (
                          <div key={`${msg.sentAt}-${index}`} className="p-3 bg-muted/50 rounded-lg animate-in fade-in duration-300">
                              <div className="flex justify-between items-start">
                                  <div>
                                      <p className="font-bold text-foreground">{msg.name}</p>
                                      <p className="text-sm text-muted-foreground">To: +{msg.number}</p>
                                  </div>
                                  <p className="text-xs text-muted-foreground whitespace-nowrap">
                                      {new Date(msg.sentAt).toLocaleString()}
                                  </p>
                              </div>
                              <p className="mt-2 p-3 text-sm bg-card rounded-md whitespace-pre-wrap">{msg.personalizedMessage}</p>
                          </div>
                      ))
                  )}
              </div>
  ...
  return (
    <div className="min-h-screen w-full bg-background flex flex-col font-sans">
      <header className="bg-primary text-primary-foreground p-4 shadow-lg flex items-center gap-4 flex-shrink-0">
        <Logo className="w-10 h-10"/>
        <h1 className="text-xl font-bold">Message Scheduler</h1>
      </header>
      
      <div className="bg-card border-b flex flex-shrink-0">
      ...
      {activeTab === 'scheduler' && (
        <footer className="p-4 bg-card border-t flex-shrink-0 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] dark:shadow-[0_-2px_10px_rgba(0,0,0,0.2)]">
          {errorMessage && <div className="p-3 mb-4 bg-destructive/10 border border-destructive/20 text-destructive rounded-lg text-sm animate-in fade-in">{errorMessage}</div>}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <Button onClick={handleSchedule} disabled={status !== AppStatus.IDLE || recipients.length === 0} variant="outline"><ScheduleIcon className="w-5 h-5 mr-2"/>Schedule</Button>
            <Button onClick={handleSendNow} disabled={status !== AppStatus.IDLE || recipients.length === 0} variant="default"><SendIcon className="w-5 h-5 mr-2"/>Send Now</Button>
          </div>
        </footer>
      )}
    </div>
